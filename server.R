...
shinyServer(function(input, output) {
  ...
  maizemat2<- reactive({
    ...
    output$pca<- renderPlot({
      ...
      if(input$ColorBy=="Grupo Morfológico") ColorBy <-maizemat2()$ColorByGpTax
    ...
    output$tree<- renderPlot({
      ...
      if(input$ColorBy=="Grupo taxonómico"){ 
    ...
    # Distribution map
     ...
     output$Legend<- renderImage({
       ...
       # Define colors
       if(input$ColorBy=="Raza") Leg <-"www/landracesEng.png"
       if(input$ColorBy=="Longitud") Leg <-"www/Longlegend.png"
       if(input$ColorBy=="Latitud") Leg <- "www/latlegend.png"
       if(input$ColorBy=="Altitud") Leg <- "www/Alt.png"
       if(input$ColorBy=="Grupo ecológico") Leg <- "www/Ecolgrp.png"
       if(input$ColorBy=="Grupo Morfológico") Leg <- "www/groups.png"
       if(input$ColorBy=="Categoría de altitud") Leg <- "www/catAlt.png"
       if(input$ColorBy=="Biogeografía de México") Leg <- "www/biogeo.png"
       if(input$ColorBy=="División Florística") Leg <- ""
       if(input$ColorBy=="Biogeografía del maíz") Leg <- "www/BiogeoMaize.png"
         list(src=Leg)
            }, deleteFile = FALSE)     
